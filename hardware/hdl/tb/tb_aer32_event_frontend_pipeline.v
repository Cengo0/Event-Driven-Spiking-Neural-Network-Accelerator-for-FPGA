...
module tb_aer32_event_frontend_pipeline;
    reg clk = 1'b0;
    always #6.25 clk = ~clk;
    ...
    wire [9:0] decoded_x;
    wire [9:0] decoded_y;
    wire [7:0] decoded_channel;
    wire decoded_raw_polarity;
    wire decoded_polarity;
    wire signed [7:0] decoded_payload;
    wire [2:0] decoded_flags;
    ...
    aer32_event_frontend_pipeline dut (
        .clk(clk),
        .rst_n(rst_n),
        .enable(enable),
        .s_aer_valid(s_aer_valid),
        .s_aer_word(s_aer_word),
        .s_timestamp_delta(s_timestamp_delta),
        .s_aer_ready(s_aer_ready),
        ...
        .decoded_raw_polarity(decoded_raw_polarity),
        .decoded_polarity(decoded_polarity),
    ...
    task automatic drive_aer;
        input [31:0] word;
        input [11:0] timestamp_delta;
        begin
            @(negedge clk);
            s_aer_word = word;
            s_timestamp_delta = timestamp_delta;
            s_aer_valid = 1'b1;
            #1;
            check("AER32 frontend accepts event when ingress is ready", s_aer_ready == 1'b1);
            @(posedge clk);
            #1;
            @(negedge clk);
            s_aer_valid = 1'b0;
    ...
    task automatic wait_for_output;
        ...
        begin
            ...
            check("AER32 frontend emits mapped event", m_event_valid == 1'b1);
        end
    endtask
    ...
    initial begin
        ...
        check("AER32 frontend reset clears output", m_event_valid == 1'b0);
        check("AER32 frontend reset clears counters", filter_input_event_count == 32'd0 && mapper_mapped_event_count == 32'd0);

        drive_aer(make_aer32(10'd4, 10'd6, 8'd1, 1'b1, 3'd5), 12'd8);
        wait_for_output();
        check("AER32 frontend exposes decoded fields", decoded_x == 10'd4 && decoded_y == 10'd6 && decoded_channel == 8'd1);
        check("AER32 frontend preserves raw flags and positive polarity", decoded_flags == 3'd5 && decoded_raw_polarity == 1'b1 && decoded_polarity == 1'b1);
        check("AER32 frontend maps positive payload", decoded_payload == 8'sd7);
        check("AER32 frontend pools coordinates before mapping", m_event_word[47:38] == 10'd2 && m_event_word[37:28] == 10'd3);
        check("AER32 frontend preserves layer and payload sideband", m_event_word[18:11] == 8'd7 && m_event_word[10:5] == 6'd2);
        check("AER32 frontend rewrites tile id after pooling", m_event_word[4:0] == 5'd5);
        check("AER32 frontend mapper derives physical IDs", m_group_id == 4'd5 && m_local_id == 7'd50 && m_global_id == 11'd690);
        check("AER32 frontend mapper exposes weight/exc", m_weight == 8'd7 && m_exc == 1'b1);
        check("AER32 frontend updates time-surface", time_surface_update_count == 32'd1 && m_time_surface_cell_index == 8'd50);
        check("AER32 frontend counters record accepted chain", filter_accepted_event_count == 32'd1 && mapper_mapped_event_count == 32'd1);
        @(posedge clk);
        #1;

        drive_aer(make_aer32(10'd8, 10'd2, 8'd1, 1'b0, 3'd1), 12'd9);
        repeat (3) @(posedge clk);
        #1;
        check("AER32 frontend drops disallowed negative polarity", m_event_valid == 1'b0 && filter_dropped_polarity_count == 32'd1);
        check("AER32 frontend polarity drop does not update downstream", time_surface_update_count == 32'd1 && mapper_mapped_event_count == 32'd1);

        @(negedge clk);
        cfg_invert_polarity = 1'b1;
        drive_aer(make_aer32(10'd8, 10'd2, 8'd1, 1'b0, 3'd2), 12'd10);
        wait_for_output();
        check("AER32 frontend inversion accepts raw negative", decoded_raw_polarity == 1'b0 && decoded_polarity == 1'b1);
        check("AER32 frontend inverted event maps pooled physical ID", m_group_id == 4'd2 && m_local_id == 7'd20 && m_global_id == 11'd276);
        check("AER32 frontend inverted event keeps positive payload", decoded_payload == 8'sd7 && m_weight == 8'd7 && m_exc == 1'b1);
        check("AER32 frontend counters record second accepted event", filter_accepted_event_count == 32'd2 && mapper_mapped_event_count == 32'd2);
        @(posedge clk);
        #1;
        ...
        enable = 1'b0;
        s_aer_valid = 1'b1;
        s_aer_word = make_aer32(10'd1, 10'd1, 8'd0, 1'b1, 3'd0);
        #1;
        check("disabled AER32 frontend deasserts input ready", s_aer_ready == 1'b0);
        s_aer_valid = 1'b0;
