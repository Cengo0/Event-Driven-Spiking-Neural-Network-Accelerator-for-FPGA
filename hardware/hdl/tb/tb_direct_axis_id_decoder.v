...
module tb_direct_axis_id_decoder;
    ...
    direct_axis_id_decoder #(
        ...
        .explicit_raw(strict_explicit_raw),
    ...
    task check_explicit_id;
        ...
        begin
            ...
            tuser = 1'b1;
            #1;
            check("explicit direct accepted", direct_raw == 1'b1);
            ...
            check("strict explicit direct accepted", strict_direct_raw == 1'b1);
    ...
    initial begin
        ...
        check("explicit 2048 is rejected", direct_raw == 1'b0);
        check("strict explicit 2048 is invalid", strict_ingress_invalid == 1'b1);
        check("strict explicit 2048 is rejected", strict_direct_raw == 1'b0);
        ...
        check("explicit 4095 is rejected", direct_raw == 1'b0);
        check("strict explicit 4095 is invalid", strict_ingress_invalid == 1'b1);
        check("strict explicit 4095 is rejected", strict_direct_raw == 1'b0);

        neuron_id = 511;
        tvalid = 1'b1;
        tuser = 1'b0;
        #1;
        check("compatibility board-visible 511 accepted", direct_raw == 1'b1);
        check("compatibility board-visible 511 preserves dest", dest_id == 511);
        check("strict rejects unmarked board-visible 511", strict_direct_raw == 1'b0);
        check("strict marks unmarked board-visible 511 invalid", strict_ingress_invalid == 1'b1);
        check("strict disables board-visible compatibility form", strict_board_visible_raw == 1'b0);

        neuron_id = 512;
        tvalid = 1'b1;
        tuser = 1'b0;
        #1;
        check("compatibility unmarked 512 is not direct", direct_raw == 1'b0);
        check("strict rejects unmarked 512", strict_direct_raw == 1'b0);
        ...
        tuser = 1'b0;
        #1;
        check("compatibility flagged 1024 accepted", direct_raw == 1'b1);
        check("compatibility flagged 1024 strips flag", dest_id == 0);
        check("strict rejects compatibility flagged 1024", strict_direct_raw == 1'b0);
        check("strict marks compatibility flagged 1024 invalid", strict_ingress_invalid == 1'b1);
        check("strict disables flagged compatibility form", strict_flagged_raw == 1'b0);

        neuron_id = 1535;
        tvalid = 1'b1;
        tuser = 1'b0;
        #1;
        check("compatibility flagged 1535 accepted", direct_raw == 1'b1);
        check("compatibility flagged 1535 maps to 511", dest_id == 511);
        check("strict rejects compatibility flagged 1535", strict_direct_raw == 1'b0);
        ...
        check("compatibility 2048 is rejected", direct_raw == 1'b0);
        check("strict rejects unmarked 2048", strict_direct_raw == 1'b0);
        ...
        tuser = 1'b1;
        #1;
        check("invalid when TVALID low does not accept", direct_raw == 1'b0);
        check("strict invalid held low when TVALID low", strict_ingress_invalid == 1'b0);
        check("strict does not accept when TVALID low", strict_direct_raw == 1'b0);
