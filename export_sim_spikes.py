...
def main():
    ...
    # Load the deployment package generated by prepare_10class_deployment.py
    data = np.load('data/cache/mnist_10class_deployment.npz', allow_pickle=True)
    q_weights = data['q_weights']
    test_imgs = data['test_imgs']
    
    n_images = 10 # We will simulate the first 10 images for the baseline
    
    # Dynamically extract parameters based on the actual keys in the archive
    n_output = q_weights.shape[0] # Matches the 150 output neurons
    pixel_threshold = float(data['pixel_threshold']) # Typically 0.3
