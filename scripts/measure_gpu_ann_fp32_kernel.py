#!/usr/bin/env python3
"""Measure GPU FP32 ANN kernel baseline (784->150, class-group argmax)."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="GPU FP32 ANN kernel benchmark")
...
def compile_bench(repo_root: Path, build_dir: Path) -> Path:
    src = repo_root / "scripts" / "gpu_ann_fp32_kernel_bench.cu"
    bin_path = build_dir / "gpu_ann_fp32_kernel_bench"
    subprocess.check_call(["nvcc", "-O3", "-std=c++17", str(src), "-o", str(bin_path)])
    return bin_path


def run_bench(bin_path: Path, imgs_bin: Path, labels_bin: Path, weights_bin: Path, n: int, nin: int, nneurons: int, fps: int, batch: int, warmup: int, repeat: int) -> dict:
    cmd = [
        str(bin_path),
        "--imgs", str(imgs_bin),
...
def main() -> None:
    ...
    q_weights = dep["q_weights"].astype(np.float32)
    weight_scale = float(dep["weight_scale"])
    weights_fp32 = q_weights / weight_scale
    ...
    nneurons = int(weights_fp32.shape[0])

    print(f"GPU FP32 ANN kernel benchmark: n={n}, nin={nin}, nneurons={nneurons}, fps={fps}")

    with tempfile.TemporaryDirectory(prefix="gpu_ann_fp32_") as td:
        td_path = Path(td)
        imgs_bin = td_path / "imgs.bin"
        labels_bin = td_path / "labels.bin"
        weights_bin = td_path / "weights_fp32.bin"

        imgs.tofile(imgs_bin)
        labels.tofile(labels_bin)
        weights_fp32.tofile(weights_bin)
        ...
        print("Running CUDA FP32 ANN kernel benchmark ...")
        t.start()
        infer = run_bench(
            bin_path=bin_path,
            imgs_bin=imgs_bin,
    ...
    result = {
        ...
        "methodology": {
            "model_type": "ann_fp32_baseline",
            "weights": "dequantized_from_q_weights",
            "topology": "784_to_150_linear_fp32",
        ...
        "inference": {
            ...
            "acc_ann_fp32": float(infer["acc_ann_fp32"]),
    ...
    print("\n=== GPU FP32 ANN Kernel Metrics ===")
    print(f"acc_ann_fp32: {result['inference']['acc_ann_fp32']*100:.2f}%")
