...
struct Args {
  std::string imgs_i8_path;
  std::string labels_path;
  std::string weights_i8_path;
  int n = 10000;
  int nin = 784;
  int nneurons = 150;
  int fps = 15;
...
bool parse_args(int argc, char** argv, Args& a) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (k == "--imgs-i8" && need(1)) a.imgs_i8_path = argv[++i];
    else if (k == "--labels" && need(1)) a.labels_path = argv[++i];
    else if (k == "--weights-i8" && need(1)) a.weights_i8_path = argv[++i];
  ...
  return !a.imgs_i8_path.empty() && !a.labels_path.empty() && !a.weights_i8_path.empty();
}
...
__global__ void potential_i8_kernel(
    const int8_t* imgs,
  ...
  const int8_t* x = imgs + static_cast<size_t>(img) * nin;
...
int main(int argc, char** argv) {
  ...
  if (!parse_args(argc, argv, a)) {
    std::cerr << "Usage: gpu_ann_int8_kernel_bench --imgs-i8 <bin> --labels <bin> --weights-i8 <bin> [--n 10000 --nin 784 --nneurons 150 --fps 15 --batch 2048 --warmup 5 --repeat 2000]" << std::endl;
    return 2;
  }

  std::vector<int8_t> h_imgs;
  std::vector<uint8_t> h_labels;
  std::vector<int8_t> h_weights;

  if (!load_bin<int8_t>(a.imgs_i8_path, h_imgs, static_cast<size_t>(a.n) * a.nin)) {
    std::cerr << "Failed to load imgs_i8" << std::endl;
    return 3;
  }
  ...
  if (!load_bin<int8_t>(a.weights_i8_path, h_weights, static_cast<size_t>(a.nneurons) * a.nin)) {
    std::cerr << "Failed to load weights_i8" << std::endl;
    return 5;
  }

  int8_t* d_imgs = nullptr;
  int8_t* d_weights = nullptr;
  int32_t* d_pot = nullptr;
  int32_t* d_pred = nullptr;

  CUDA_CHECK(cudaMalloc(&d_imgs, h_imgs.size() * sizeof(int8_t)));
  ...
  CUDA_CHECK(cudaMemcpy(d_imgs, h_imgs.data(), h_imgs.size() * sizeof(int8_t), cudaMemcpyHostToDevice));
  ...
  for (int i = 0; i < a.warmup; ++i) {
    potential_i8_kernel<<<grid_p, block_p>>>(d_imgs, d_weights, d_pot, warm_n, a.nin, a.nneurons);
  ...
  for (int r = 0; r < repeat; ++r) {
    for (int s = 0; s < a.n; s += a.batch) {
      ...
      const int8_t* d_imgs_s = d_imgs + static_cast<size_t>(s) * a.nin;
      ...
      potential_i8_kernel<<<g_p, block_p>>>(d_imgs_s, d_weights, d_pot, cur, a.nin, a.nneurons);
