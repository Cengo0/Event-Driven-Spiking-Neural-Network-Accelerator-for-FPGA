...
__global__ void potential_kernel(
  ...
  const float* w = weights + static_cast<size_t>(neuron) * nin;
  float sum = 0.0f;
  for (int i = 0; i < nin; ++i) {
    sum += x[i] * w[i];
  }
...
__global__ void argmax_kernel(const float* potentials, int32_t* pred_cls, int nimgs, int nneurons, int fps) {
  int img = blockIdx.x * blockDim.x + threadIdx.x;
  if (img >= nimgs) return;

  const float* p = potentials + static_cast<size_t>(img) * nneurons;
  int best_idx = 0;
  float best_val = p[0];
...
int main(int argc, char** argv) {
  ...
  if (!parse_args(argc, argv, a)) {
    std::cerr << "Usage: gpu_ann_fp32_kernel_bench --imgs <bin> --labels <bin> --weights <bin> [--n 10000 --nin 784 --nneurons 150 --fps 15 --batch 2048 --warmup 5 --repeat 2000]" << std::endl;
    return 2;
  }
  ...
  if (!load_bin<float>(a.weights_path, h_weights, static_cast<size_t>(a.nneurons) * a.nin)) {
  ...
  CUDA_CHECK(cudaMalloc(&d_pot, static_cast<size_t>(a.batch) * a.nneurons * sizeof(float)));
  ...
  for (int i = 0; i < a.warmup; ++i) {
    potential_kernel<<<grid_p, block_p>>>(d_imgs, d_weights, d_pot, warm_n, a.nin, a.nneurons);
  ...
  for (int r = 0; r < repeat; ++r) {
    for (int s = 0; s < a.n; s += a.batch) {
      ...
      potential_kernel<<<g_p, block_p>>>(d_imgs_s, d_weights, d_pot, cur, a.nin, a.nneurons);
